...
void MaxFileLoader::RenderNode(Lib3dsNode *iNode)
{
  ...
  if (iNode->type==LIB3DS_OBJECT_NODE) 
  {
    ...
    if (!iNode->user.p) // If the shape is not built yet, just do it !
    {
      ...
      // builds the shape:
      shape = new NodeShape;
      iNode->user.p=(void*)shape; // We store as user data the NodeShape address
