...
namespace Config{
  ...
  QString Path::getEnvVar(const QString& iEnvVarName){
    QString value;
    if (!getenv(iEnvVarName.toStdString().data())) {
      cerr << "Warning: You may want to set the $"<< iEnvVarName.toStdString().data()
		  << " environment variable to use " << QString(Config::APPLICATION_NAME).toStdString().data() << "." << endl
	    << "         Otherwise, the current directory will be used instead." << endl;
      value = ".";
    }else{
      value = getenv(iEnvVarName.toStdString().data());
    } 
    return value;
  }
