...
//   _Grid.displayDebug();
  ...
  string basename((const char*)qfi.fileName().toStdString().data());
...
//  #endif
  ...
  ofs << Config::VIEWMAP_MAGIC.toStdString().data() << endl << Config::VIEWMAP_VERSION.toStdString().data() << endl;
...
//  #endif
  ...
  if (test != Config::VIEWMAP_MAGIC) {
    _pMainWindow->DisplayMessage(
		(QString("Error: This is not a valid .") + Config::VIEWMAP_EXTENSION + QString(" file")).toStdString().data());
    cerr << "Error: This is not a valid ." << Config::VIEWMAP_EXTENSION.toStdString().data() << " file" << endl;
    return;
  }
  ifs.getline(tmp_buffer, 255);
  test = tmp_buffer;
  if (test != Config::VIEWMAP_VERSION && !only_camera) {
    _pMainWindow->DisplayMessage(
		(QString("Error: This version of the .") + Config::VIEWMAP_EXTENSION + QString(" file format is no longer supported")).toStdString().data());
    cerr << "Error: This version of the ." << Config::VIEWMAP_EXTENSION.toStdString().data() << " file format is no longer supported" << endl;
    return;
  }
...
void Controller::InsertStyleModule(unsigned index, const char *iFileName)
{
  ...
  if (ext != "py") {
    cerr << "Error: Cannot load \"" << fi.fileName().toStdString().data()
...
void Controller::savePSSnapshot(const QString& iFileName){
  PSStrokeRenderer psRenderer((const char*)iFileName.toStdString().data());
  _Canvas->Render(&psRenderer);
  psRenderer.Close();
}

void Controller::saveTextSnapshot(const QString& iFileName){
  TextStrokeRenderer textRenderer((const char*)iFileName.toStdString().data());
