  ...
  if ((ext == "3ds") || (ext == "3DS"))
    {
      ...
      g_pController->Load3DSFile(s.toStdString().data()); // lunch time...
    ...
    {
      QApplication::setOverrideCursor( Qt::WaitCursor );
      g_pController->LoadViewMapFile(s.toStdString().data()); // ...and now tea time...
      g_pController->setModelsDir(fi.dir().path());
      QApplication::restoreOverrideCursor();
    }
}
...
void AppMainWindow::loadCamera()
{
  ...
  if (ext == Config::VIEWMAP_EXTENSION)
    {
      QApplication::setOverrideCursor( Qt::WaitCursor );
      g_pController->LoadViewMapFile(s.toStdString().data(), true);
      QApplication::restoreOverrideCursor();
    }
}
...
void AppMainWindow::ViewMapFileSave() {
  ...
  QApplication::setOverrideCursor( Qt::WaitCursor );
  g_pController->setModelsDir(fi.dir().path());
  g_pController->SaveViewMapFile(s.toStdString().data());
  QApplication::restoreOverrideCursor();
}
