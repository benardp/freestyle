  ...
  : QGLViewer(iParent)
{
  ...
  qglviewer::ManipulatedCameraFrame *  fr = cam->frame() ;
...
AppGLWidget::keyPressEvent(QKeyEvent* e)
{
  ...
  case Qt::Key_M: 
    ...
    update(); break;
  case Qt::Key_Plus:
    Canvas::getInstance()->changePaperTexture(true);update();
    break;
  case Qt::Key_Minus:
    Canvas::getInstance()->changePaperTexture(false);update();
    break;
  case Qt::Key_P:
    Canvas::getInstance()->togglePaperTexture();update();
    break;
  case Qt::Key_PageUp:
  ...
  case Qt::Key_PageDown:
    ...
    update();
    break;
  case Qt::Key_1: _ModelRootNode->SetStyle(DrawingStyle::FILLED); update(); break;
  case Qt::Key_2: _ModelRootNode->SetStyle(DrawingStyle::LINES); _ModelRootNode->SetLineWidth(1.0); update(); break;
  case Qt::Key_3: _ModelRootNode->SetStyle(DrawingStyle::INVISIBLE); update(); break;
  case Qt::Key_B:
    {
//       if(e->state() == ShiftButton)
//         {
//           g_pController->toggleEdgeTesselationNature(Nature::BORDER); update(); break;
//         }
//       else
        {
...
//     if(e->state() == ShiftButton)
//       {
// 	g_pController->toggleEdgeTesselationNature(Nature::CREASE); update(); break;
//       }
//     break;
  case Qt::Key_S:
    {
//       if(e->state() == ShiftButton)
//         {
//           g_pController->toggleEdgeTesselationNature(Nature::SILHOUETTE); update(); break;
//         }
//       else
        {
          _silhouette == true ? _silhouette = false : _silhouette = true; update(); break;
        }
    }
  case Qt::Key_L: 
    {
        _selection_mode = !_selection_mode; update(); break;
    }
    break;
  case Qt::Key_E: 
    {
        _fedges == true ? _fedges = false : _fedges = true; update(); break;
    }
    break;
  case Qt::Key_D: 
    {
      _debug == true ? _debug = false : _debug = true; update();
    }
    break;
  case Qt::Key_F2: _Draw2DScene == true ? _Draw2DScene = false : _Draw2DScene = true; update(); break; 
  case Qt::Key_F3: _Draw3DScene == true ? _Draw3DScene = false : _Draw3DScene = true; update(); break; 
  default:
...
//  setShortcut(QGLViewer::DISPLAY_Z_BUFFER, 0);
  ...
  //trackball().fitBBox(_ModelRootNode->bbox().getMin(), _ModelRootNode->bbox().getMax(), _Fovy);
   ...
   LoadEnvMap((envmapDir + QString("gray00.png")).toStdString().data());
   //LoadEnvMap(Config::ENV_MAP_DIR + "gray01.bmp");
   LoadEnvMap((envmapDir + QString("gray02.png")).toStdString().data());
   LoadEnvMap((envmapDir + QString("gray03.png")).toStdString().data());
   LoadEnvMap((envmapDir + QString("brown00.png")).toStdString().data());
