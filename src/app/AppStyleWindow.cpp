...
void AppStyleWindow::AddList(const char* iFileName) {
  ...
  while (!ifs.eof()) {
    ...
    if (tmp_buffer[0] == '0')
      ...
      disp = true;
    s = (const char*)fi.dir().path().toStdString().data();
    s += Config::DIR_SEP;
    s += tmp_buffer + 1;
...
void AppStyleWindow::SaveList() {
  ...
  ofstream ofs(s.toStdString().data(), ios::binary);
  if (!ofs.is_open()) {
  ...
  QTableWidgetItem *checkItem;
    ...
    ofs << PlayList->item(i, 1)->text().toStdString().data() << endl;
  }
...
void AppStyleWindow::Add()
{
  ...
  if (ext == Config::STYLE_MODULE_EXTENSION) {
    g_pController->setModulesDir(fi.dir().path());
    Add(s.toStdString().data());
  }
  else if (ext == Config::STYLE_MODULES_LIST_EXTENSION) {
    g_pController->setModulesDir(fi.dir().path());
    AddList(s.toStdString().data());
  }
}
...
void AppStyleWindow::fileSave() {
  ...
  g_pController->ReloadStyleModule(current, text.toStdString().data());
